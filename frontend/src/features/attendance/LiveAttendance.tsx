...
export const LiveAttendance: React.FC = () => {
  ...
  useEffect(() => {
    if (!isReady || employees.length === 0 || isScanComplete) return;
      ...
      <div className="w-full max-w-7xl mx-auto flex items-center justify-center p-6 gap-8">
        ...
        <div className="flex-1 max-w-3xl">
          <div className="relative w-full aspect-[4/3] bg-slate-200 rounded-[2rem] overflow-hidden shadow-2xl shadow-violet-900/10 border-8 border-white">
            ...
            {isScanComplete && (
              <div className="absolute inset-0 z-50 flex items-center justify-center bg-slate-950/75 backdrop-blur-sm pointer-events-auto">
                <div className="text-center px-6 py-8 rounded-3xl bg-white/10 border border-white/20 shadow-2xl">
                  <h2 className="text-2xl font-bold text-white mb-4">Absensi Selesai</h2>
                  <button
                    onClick={handleNextScan}
                    className="px-5 py-3 bg-violet-600 hover:bg-violet-700 text-white rounded-lg font-semibold"
                  >
